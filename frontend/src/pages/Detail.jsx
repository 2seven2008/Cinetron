...
export default function Detail({ type }) {
  ...
  const toggleList = async () => {
    ...
    } catch { showToast('Erro ao atualizar lista', 'error') }
  }
  ...
  const addDownload = async () => {
    ...
    } catch { showToast('Já nos downloads', 'info') }
  }
  ...
  const runtime = data.runtime ? `${data.runtime}min` : data.number_of_seasons ? `${data.number_of_seasons} temporada(s)` : ''
  const genres = (data.genres || []).map(g => g.name).join(' · ')
  const trailer = (data.videos?.results || []).find(v => v.type === 'Trailer' && v.site === 'YouTube')
  const cast = (data.credits?.cast || []).slice(0, 10)
  const similar = (data.similar?.results || data.recommendations?.results || []).slice(0, 10)
            ...
            <div className="detail-info">
              ...
              <div className="detail-actions">
                {trailer ? (
                  ...
                  <button className="btn-watch" onClick={() => showToast('Player em breve! 🚀', 'info')}>
                    <Play size={16} fill="currentColor" /> Assistir
                  </button>
                )}
                <button className={`btn-icon ${inList ? 'btn-icon--active' : ''}`} onClick={toggleList} title={inList ? 'Remover da lista' : 'Adicionar à lista'}>
                  {inList ? <Check size={18} /> : <Plus size={18} />}
                </button>
                <button className="btn-icon" onClick={addDownload} title="Download">
                  <Download size={18} />
                </button>
              </div>
                ...
                {(seasonData.episodes || []).map(ep => (
                  <div key={ep.id} className="episode-card">
                    ...
                    <div className="episode-info">
                      ...
                      {ep.runtime && <p className="episode-runtime">{ep.runtime}min</p>}
                      {ep.overview && <p className="episode-overview">{ep.overview.slice(0, 100)}...</p>}
                    </div>
                    <button className="episode-play" onClick={() => showToast('Player em breve! 🚀', 'info')}>
