...
export default function Buscar() {
  ...
  const goDetail = (item) => {
    const type =
      item.media_type === "tv" || item.first_air_date ? "serie" : "filme";
    nav(`/${type}/${item.id}`);
  };
  ...
  return (
    <div className="page page--search fade-up">
          ...
          <div className="search-section">
            ...
            <div className="tags">
              {["Comédia", "Terror", "Ação", "Drama", "Animação"].map((t) => (
                <button key={t} className="tag" onClick={() => setQuery(t)}>
                  {t}
                </button>
              ))}
            </div>
      ...
      {!loading && displayResults.length > 0 && (
        <div className="search-section">
          {selectedGenre && (
            <h3 className="search-section-title">{selectedGenre.name}</h3>
          )}
          <div className="grid-2">
            {displayResults
              .filter((r) => {
                if (tab === "filmes")
                  return !r.first_air_date && r.media_type !== "tv";
                if (tab === "series")
                  return !!r.first_air_date || r.media_type === "tv";
                return true;
              })
